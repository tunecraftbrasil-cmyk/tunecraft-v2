...
type AlterarLetraRequest = {
  pedidoId: string;
  sugestoes: string | Record<string, unknown>;
};
...
function describeSectionAction(section: { action?: string; text?: string }) {
  const detail = (section.text || "").trim();
  if (section.action === "keep") {
    return "Manter exatamente como está.";
  }
  if (section.action === "alter") {
    return detail
      ? `Fazer ajustes pontuais conforme: ${detail}`
      : "Fazer ajustes pontuais conforme o solicitado.";
  }
  if (section.action === "rewrite") {
    return detail
      ? `Reescrever por completo considerando: ${detail}`
      : "Reescrever esse trecho por completo.";
  }
  return detail || "Modificar conforme solicitado.";
}

function buildStructuredSuggestionText(payload: any) {
  if (!payload || typeof payload !== "object") return "";
  const lines: string[] = [];

  if (payload.title) {
    const titleAction =
      payload.title.action === "keep"
        ? "Manter o título."
        : payload.title.action === "alter"
        ? `Ajustar o título${payload.title.text ? ` (${payload.title.text})` : ""}.`
        : `Reescrever o título${payload.title.text ? ` (${payload.title.text})` : ""}.`;
    lines.push(`Título: ${titleAction}`);
  }

  if (Array.isArray(payload.sections)) {
    payload.sections.forEach((section: any, index: number) => {
      const label = section.label || `Trecho ${index + 1}`;
      const actionText = describeSectionAction(section);
      const preview = (section.original || "")
        .split("\n")
        .map((line: string) => line.trim())
        .filter(Boolean)
        .slice(0, 1)
        .join(" - ");
      lines.push(`${label}: ${actionText}${preview ? ` Trecho atual: "${preview}..."` : ""}`);
    });
  }
...
function buildSuggestionText(rawSuggestion: string | object | undefined) {
  if (!rawSuggestion) return "";
  if (typeof rawSuggestion === "string") {
    try {
      const parsed = JSON.parse(rawSuggestion);
      return buildStructuredSuggestionText(parsed) || rawSuggestion;
    } catch {
      return rawSuggestion;
    }
  }
  return buildStructuredSuggestionText(rawSuggestion) || String(rawSuggestion);
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!sugestoes) {
      return jsonResponse({ error: "Sugestões obrigatórias são necessárias para orientar o GPT" }, 400);
    }

    const suggestionText = buildSuggestionText(sugestoes) || "Sem instruções específicas além de manter o tom original.";
    ...
    console.log("🧭 Sugestões formatadas:", suggestionText);
    ...
    console.log("💬 Sugestões recebidas:", suggestionText.substring(0, 100) + "...");
...
- SE A SUGESTÃO FOR CONTRADITÓRIA OU GENÉRICA
...
- Mantenha o estilo musical (não mude o "style").
    ...
    const versaoModificada = {
      ...
      style: versaoOriginal.style,
      suno_payload: {
        title: modelJson.suno_payload.title,
        style: versaoOriginal.suno_payload?.style || modelJson.suno_payload.style,
